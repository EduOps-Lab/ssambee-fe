...
export const manualStudentSchema = z.object({
  ...
  school: z.string().min(1, "학생 학교를 입력해주세요"),
  studentGrade: z.string().min(1, "학생 학년을 입력해주세요"),
  parentPhone: z
    .string()
    .min(1, "학부모 번호를 입력해주세요")
    .regex(/^010-?\d{4}-?\d{4}$/, "올바른 연락처 형식이 아닙니다"),
  registrationDate: z.string().min(1, "학생 등록날짜를 입력해주세요"),
});
...
export const lectureInfoSchema = z.object({
  ...
  startDate: z.string().min(1, "개강일을 입력해주세요"),
